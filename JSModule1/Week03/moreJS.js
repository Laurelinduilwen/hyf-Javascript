...
function sumThemAll(x, y, z) {
  console.log((i = x + y + z));
}

sumThemAll(345, 18345, 6756);
...
function resultSentence() {
  let resultString = ``;
  for (i = 0; i < listOfVehicles.length - 1; i++) resultString += listOfVehicles[i] + 's, ';
...
let amazingJoe = "Amazing Joe's Garage, we service " + resultSentence();
...
//edit- much more efficient with the function now :D

amazingJoe = "Amazing Joe's Garage, we service " + resultSentence();
...
const teachers = [
  new Teacher('Philipp', 'HTML-CSS'),
  new Teacher('Igor', 'CLI & GIT'),
  new Teacher('Nick', 'JavaScript'),
];
...
edit - when we assign an array to another array like this we are simply changing what object the variable refers to.
in this case when we assign y to z like this we dont create a new copy. 
*/
y.pop();
...
//when we console log z we can see that it is actually referencing y

z.push(99);

console.log(y);

//we pushed to z but logged y :D I think this clearly demonstrates the idea.
...
//edit - I also demonstrated the idea in detail in the previous question.
